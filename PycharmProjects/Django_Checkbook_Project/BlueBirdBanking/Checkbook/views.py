...
def home(request):
    from = TransactionForm(data=request.POST or None)  # Retrieve Transaction form
    # Checks if request method in POST
    ...
    return render(request, 'checkbook/index',content)
...
def create_account(request):
    ...
    if request.method == 'POST':
        ...
        # adds content of form to page
        return render(request, 'CreateNewAccount.html', content)
...
def balance(request, pk):
    ...
    table_contents = {} # Create a dictionary into with transaction information will be placed
...
def transaction(request):
    ...
    # Checks if request method is POST
    if request.method == 'Post':
        if form.is_valid():  # Check to see if the submitted form is valid and id so, saves the form
